 ...
 * lead to a {@link RadioDetailActivity} representing
...
public class CategoriesListActivity extends Activity
    ...
    public void onItemSelected(int id) {
        if (mTwoPane) {
            ...
            arguments.putInt(RadioDetailFragment.ARG_ITEM_ID, id);
            RadioDetailFragment fragment = new RadioDetailFragment();
            fragment.setArguments(arguments);
            getFragmentManager().beginTransaction()
                    .replace(R.id.radio_detail_container, fragment)
                    .commit();

        } else {
            // In single-pane mode, simply start the detail activity
            // for the selected item ID.
            Intent detailIntent = new Intent(this, RadioDetailActivity.class);
            detailIntent.putExtra(RadioDetailFragment.ARG_ITEM_ID, id);
